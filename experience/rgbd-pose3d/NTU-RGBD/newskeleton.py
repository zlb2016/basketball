...
class Draw3DSkeleton(object):
    ...
    def __init__(self, file, save_path='./NTU/out_skeleton', init_horizon=-45,
      ...
      #      os.mkdir(self.save_path)
        self.xy = self.read_xy(self.file)#读取彩色图2D坐标
    ...
    def read_xyz(self, file, max_body=2, num_joint=25):#读取x,y,z坐标
        seq_info = self._read_skeleton(file)
        print('seq_info--=-=',seq_info['numFrame'])
    ...
    def read_xy(self, file, max_body=2, num_joint=25):#读取x,y坐标
        seq_info = self._read_skeleton(file)
        print('seq_info--=-=',seq_info['numFrame'])
        data = np.zeros((3, seq_info['numFrame'], num_joint, max_body))  # (3,frame_nums,25 2)
        for n, f in enumerate(seq_info['frameInfo']):
            for m, b in enumerate(f['bodyInfo']):
                for j, v in enumerate(b['jointInfo']):
                    if m < max_body and j < num_joint:
                        data[:, n, j, m] = [v['colorX'], v['colorY'],1]#(x,y,score)
                    else:
                        pass
        return data
    ...
    def write_2D_file(self,data):#写入2D坐标
        dic = {}
        # show every frame 3d skeleton
        with open('./truth_ntu_008_2d.json','a') as f:
            for frame_idx in range(data.shape[1]):
                dic['person_num']='008'
                dic['frame_index']=frame_idx
                x = data[0, frame_idx, :, 0]
                y = data[0, frame_idx, :, 1]
                score = data[0,frame_idx,:,2]
                print('x',x)
                print('y',y)
                print('score',score)
                s=[]
                for i in range(25):#合并成x,y格式
                    s.append([x[i],y[i],score[i]])
                print('s',s)
                dic['2D_skeleton']=s
    ...
    def visual_skeleton(self):
        ...
        # dataed=self.xy
        dataed = np.transpose(self.xy, (3, 1, 2, 0))# 调换索引值，将x下标的索引值换到最后，将max_body索引值换到前面来
        print('data.shape[1]',dataed.shape[1])
        self.write_2D_file(dataed)
        # print('dataed--',dataed.shape)
        # print('====',dataed[0,0,0,:])
        ...
        # dic = {}
        # # show every frame 3d skeleton
        # with open('./truth_ntu_008_3d.json','a') as f:
        #     for frame_idx in range(data.shape[1]):
        #         dic['person_num']='008'
        #         dic['frame_index']=frame_idx
        #         plt.cla()
        ...
        #         print('z',z)
        #         s=[]
        #         for i in range(25):#合并成x,y,z格式
        #             s.append([x[i],y[i],z[i]])#坐标是统一处理后的，不是原先的坐标
        #         print('s',s)
        #         dic['skeleton']=s
        #         for part in body:
        #             x_plot = x[part]
...
if __name__ == '__main__':
    # test sample
    sk = Draw3DSkeleton("./NTU/raw_txt120/S018C002P008R002A063.skeleton")
    sk.visual_skeleton()
