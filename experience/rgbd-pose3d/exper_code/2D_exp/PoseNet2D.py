...
from OpenPoseCoco import *


class PoseNet2D(object):
    ...
    def detect(self, image):
        """ Given RGB input it predicts 3D human pose. """
        ...
        #transform person_det['person0']['kp']=list
        kk=[]
        for j in range(len(person_det)):#people num
          for i in range(18):#keypoints 
              kk.append(person_det['person'+str(j)]['kp'][i].tolist())
          print('kk',kk)
          person_det['person'+str(j)]['kp']=kk
        ...
        return person_det, keypoint_det_fs
