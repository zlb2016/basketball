...
class PoseNet3D(object):
    def __init__(self, ope_depth=5, gpu_id=0, gpu_memory_limit=None, vpn_type=None, K=None):
        ...
        self._setup_network()  # creates the tensorflow graph)创建tensor
        self._init_open_pose()  # loads weights加载权重文件
    ...
    def detect(self, image, depth_w, mask):
        ...
        coord_uv, coord2d_conf = self._trafo_dict2array(person_det)#person_det:coor{'person0': {'kp': [[376.       ,  32.       ,   0.8929911]]}}
        coord_vis = coord2d_conf > self.conf2d_thresh#设置2D可见性
        ...
        for pid in range(len(person_det)):
            ...
            if coord_vis[pid, 1] == 1.0:#判断颈部关键点是否存在
            ...
            # find approx. depth for root找到颈部节点的大约深度
            ...
            # create voxel occupancy grid from the warped depth map从变形的深度图中创建体素网格
            ...
            # assemble solution from voxel result and backprojected solution从voxel结果和反投影结果中选择最终的3D得分图
    ...
    def _setup_network(self):
        ...
        # tile 2D scoremap into a scorevolume将2D得分图平铺到得分量中
    ...
    def _get_depth_value(self, map, u, v, crop_size=25):
        """ Extracts a depth value from a map.从map中提取深度值
            Checks for the closest value in a given neighborhood. 查找邻居中最近的值"""
        coord = np.array([[u, v]])
    ...
    @staticmethod
    def _crop_at(map, center_coord, size):#切割大小为25
        """ Crop a given map at the given center coordinate with the crop size specified.使用指定的大小给定中心处裁剪map
            If the cropped area would partially reside outside of the map it is translated accordingly.裁剪区域位于map外面则进行相应转换 """
        expand = False
        s = map.shape
        if len(s) == 2:
    ...
    @staticmethod
    def _trafo_dict2array(person_det, kp_conf_thresh=0.25):#设置分数阈值为0.25
    ...
    def _detect_scorevol(scorevolume):
        """ Finds maximum volumewise. Tensor scorevolume is [1, D, H, W, C].查找最大的体素，Tensor的体素量 """
